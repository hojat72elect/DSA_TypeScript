...
import {dijkstra} from '../../../../src/algorithms/graph/dijkstra/dijkstra.ts';
...
describe('dijkstra', () => {
    it('should find minimum paths to all vertices for undirected graph', () => {
        ...
        expect((previousVertices as any).F.getKey()).toBe('D');
        expect((previousVertices as any).D.getKey()).toBe('B');
        expect((previousVertices as any).B.getKey()).toBe('A');
        expect((previousVertices as any).G.getKey()).toBe('E');
        expect((previousVertices as any).C.getKey()).toBe('A');
        expect((previousVertices as any).A).toBeNull();
        expect((previousVertices as any).H).toBeNull();
    });
    ...
    it('should find minimum paths to all vertices for directed graph with negative edge weights', () => {
        ...
        expect((previousVertices as any).H).toBeNull();
        expect((previousVertices as any).S).toBeNull();
        expect((previousVertices as any).B.getKey()).toBe('C');
        expect((previousVertices as any).C.getKey()).toBe('A');
        expect((previousVertices as any).A.getKey()).toBe('D');
        expect((previousVertices as any).D.getKey()).toBe('E');
    });
});
