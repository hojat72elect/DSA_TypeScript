import {hillCipherDecrypt, hillCipherEncrypt} from '../../../../src/algorithms/cryptography/hill-cipher/hillCipher.ts';
...
describe('hillCipher', () => {
    ...
    it('should throw an error when message or keyString contains none letter character', () => {
        ...
        expect(invalidCharacterInMessage).toThrow('The message and key string can only contain letters',);
        expect(invalidCharacterInKeyString).toThrow('The message and key string can only contain letters',);
    });
    ...
    it('should throw an error when the length of the keyString has a square root which is not integer', () => {
        ...
        expect(invalidLengthOfKeyString).toThrow('Invalid key string length. The square root of the key string must be an integer',);
    });

    it('should throw an error when the length of the keyString does not equal to the square of length of the message', () => {
        const invalidLengthOfKeyString = () => {
            hillCipherEncrypt('ab', 'aaabbbccc');
        };
        expect(invalidLengthOfKeyString).toThrow('Invalid key string length. The key length must be a square of message length',);
    });
