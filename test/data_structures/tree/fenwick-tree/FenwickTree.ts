export class FenwickTree {
    ...
    /**
     * creates empty Fenwick tree of size `arraySize`,
    ...
    /**
     * Adds value to the existing value at position.
     */
    increase(position: number, value: number): FenwickTree {
        if (position < 1 || position > this.arraySize) throw new Error('Position is out of allowed range');
    ...
    queryRange(leftIndex: number, rightIndex: number): number {
        if (leftIndex > rightIndex) throw new Error('Left index can not be greater than right one');
        if (leftIndex === 1) return this.query(rightIndex);
