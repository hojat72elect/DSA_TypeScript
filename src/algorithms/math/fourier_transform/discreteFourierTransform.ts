import ComplexNumber from '../complex_number/ComplexNumber';
 ...
 */
export default function dft(inputAmplitudes: number[], zeroThreshold: number = CLOSE_TO_ZERO_THRESHOLD): ComplexNumber[] {
    const N: number = inputAmplitudes.length;
    const signals: ComplexNumber[] = [];

    // Go through every discrete frequency.
    for (let frequency: number = 0; frequency < N; frequency += 1) {
        // Compound signal at current frequency that will ultimately
        // take part in forming input amplitudes.
        let frequencySignal: ComplexNumber = new ComplexNumber();

        // Go through every discrete point in time.
        for (let timer: number = 0; timer < N; timer += 1) {
            const currentAmplitude: number = inputAmplitudes[timer]!;

            // Calculate rotation angle.
            const rotationAngle: number = -1 * (2 * Math.PI) * frequency * (timer / N);

            // Remember that e^ix = cos(x) + i * sin(x);
            const dataPointContribution: ComplexNumber = new ComplexNumber({
