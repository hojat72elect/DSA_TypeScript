 ...
 */
export function fibonacciClosedForm(position: number) {
    const topMaxValidPosition = 70;

    // Check if the position is valid
    if (position < 1 || position > topMaxValidPosition) throw new Error(`Can't handle positions smaller than 1 or greater than ${topMaxValidPosition}`);
