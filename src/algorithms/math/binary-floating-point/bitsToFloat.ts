...
export interface PrecisionConfigs {
...
function bitsToFloat(bits: Bits, precisionConfig: PrecisionConfig): number {
    ...
    const exponentBits: number[] = bits.slice(signBitsCount, signBitsCount + exponentBitsCount);
    ...
    const fractionBits: number[] = bits.slice(signBitsCount + exponentBitsCount);
