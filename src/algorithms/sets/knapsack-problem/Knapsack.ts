import {KnapsackItem} from "./KnapsackItem.ts";
import {MergeSort} from "../../sorting/merge-sort/MergeSort.ts";

export default class Knapsack {
    public selectedItems: KnapsackItem[];
    ...
    sortPossibleItemsByValue() {
        this.possibleItems = new MergeSort<KnapsackItem>({
            compareCallback: (itemA, itemB) => {
                if (itemA.value === itemB.value) return 0;
    ...
    sortPossibleItemsByValuePerWeightRatio() {
        this.possibleItems = new MergeSort<KnapsackItem>({
            compareCallback: (itemA, itemB) => {
                if (itemA.valuePerWeightRatio === itemB.valuePerWeightRatio) return 0;
    ...
    solveZeroOneKnapsackProblem() {
        ...
        while (itemIndex > 0) {
            ...
            if (
                ...
                // We need to add the highest item in the matrix that is possible to get the highest value.
