...
// Using charCode (a = 97, b = 98, etc), we can map characters to buckets from 0 to 25
const BASE_CHAR_CODE = 97;
const NUMBER_OF_POSSIBLE_DIGITS = 10;
const ENGLISH_ALPHABET_LENGTH = 26;

export default class RadixSort extends Sort<number | string> {

    sort(originalArray: (number | string)[]): (number | string)[] {
    ...
    placeElementsInNumberBuckets(array: number[], index: number): number[][] {
        // See below. These are used to determine which digit to use for bucket allocation
        const modded = 10 ** (index + 1);
        const divided = 10 ** index;
        const buckets = this.createBuckets<number>(NUMBER_OF_POSSIBLE_DIGITS);
    ...
    placeElementsInCharacterBuckets(array: string[], index: number, numPasses: number): string[][] {
        const buckets = this.createBuckets<string>(ENGLISH_ALPHABET_LENGTH);
     ...
     */
    determineNumPasses(array: (number | string)[]): number {
        return this.getLengthOfLongestElement(array);
    }

    getLengthOfLongestElement(array: (number | string)[]): number {
        if (this.isArrayOfNumbers(array)) {
            return Math.floor(Math.log10(Math.max(...array as number[]))) + 1;
        }

        return array.reduce((acc: number, val: number | string) => {
            return (val as string).length > acc ? (val as string).length : acc;
        }, -Infinity);
    }

    isArrayOfNumbers(array: (number | string)[]): boolean {
        // Assumes all elements of array are of the same type
        return this.isNumber(array[0]!);
    }

    createBuckets<T>(numBuckets: number): T[][] {
        /**
    ...
    isNumber(element: number | string): boolean {
