const ZERO_OR_MORE_CHARS: string = '*';
const ANY_CHAR: string = '.';

/**
 * Dynamic programming approach.
 */
export function regularExpressionMatching(string: string, pattern: string): boolean {
    /*
    ...
    for (let rowIndex = 1; rowIndex <= string.length; rowIndex += 1) {
        for (let columnIndex = 1; columnIndex <= pattern.length; columnIndex += 1) {
            ...
            if (pattern[patternIndex] === ZERO_OR_MORE_CHARS) {
                ...
                } else if ((pattern[patternIndex - 1] === string[stringIndex] || pattern[patternIndex - 1] === ANY_CHAR) && matchMatrix[rowIndex - 1]![columnIndex] === true) {
                    matchMatrix[rowIndex]![columnIndex] = true;
                } else {
