...
export function dijkstra(graph: Graph, startVertex: GraphVertex): ShortestPaths {
    // Specifying precise record types instead of generic object maps
    const distances: Record<string, number> = {};
    const visitedVertices: Record<string, GraphVertex> = {};
    const previousVertices: Record<string, GraphVertex | null> = {};

    // Assuming PriorityQueue accepts generic types, or handles objects gracefully
    ...
    // Init vertices queue.
    queue.addItemWithPriority(startVertex, distances[startVertex.getKey()]);

    // Iterate over the priority queue of vertices until it is empty.
    while (!queue.isEmpty()) {
        // Fetch next closest vertex.
        const currentVertex = queue.poll() as GraphVertex;
        ...
        currentVertex.getNeighbors().forEach((neighbor) => {
            ...
            if (!visitedVertices[neighbor.getKey()]) {
                ...
                if (distanceToNeighborFromCurrent < existingDistanceToNeighbor!) {
                    ...
                    // Change priority of the neighbor in a queue since it might have become closer.
                ...
                if (!queue.hasValue(neighbor)) {
                    queue.addItemWithPriority(neighbor, distances[neighbor.getKey()]);
                }
            }
        });
