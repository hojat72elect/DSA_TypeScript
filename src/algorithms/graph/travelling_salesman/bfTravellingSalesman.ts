 ...
 */
function findAllPaths(startVertex: GraphVertex, paths: GraphVertex[][] = [], path: GraphVertex[] = []): GraphVertex[][] {
    // Clone path.
    ...
    const visitedSet = currentPath.reduce<Record<string, GraphVertex>>((accumulator, vertex) => {
    ...
    // If there are no unvisited neighbors then treat current path as complete and save it.
...
function getCycleWeight(adjacencyMatrix: number[][], verticesIndices: Record<string, number>, cycle: GraphVertex[]): number {
    let weight = 0;
 ...
 */
export function bfTravellingSalesman(graph: Graph): GraphVertex[] {
    ...
    let salesmanPathWeight: number | null = null;
