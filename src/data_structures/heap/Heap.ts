...
export abstract class Heap<T> {
    ...
    poll(): T | null {
        if (this.heapContainer.length === 0) return null;
        if (this.heapContainer.length === 1) return this.heapContainer.pop() ?? null;

        const item = this.heapContainer[0];

        //Move the last element from the end of the heap to te head.
    ...
    remove(item: T, comparator: Comparator = this.compare): Heap<T> {
        ...
        for (let iteration = 0; iteration < numberOfItemsToRemove; iteration += 1) {
            ...
            if (indexToRemove === undefined) continue;
