...
See the `LRUCache` implementation example in [LRUCache.ts](./LRUCache.ts). The solution uses a `HashMap` for fast `O(1)` (in average) cache items access, and a `DoublyLinkedList` for fast `O(1)` (in average) cache items promotions and eviction (to keep the maximum allowed cache capacity).

![Linked List](./images/lru-cache.jpg)

_Made with [okso.app](https://okso.app)_

You may also find more test-case examples of how the LRU Cache works in [LruCache.test.ts](./__test__/LruCache.test.ts) file.
...
See the `LRUCacheOnMap` implementation example in [LRUCacheOnMap.ts](./LRUCacheOnMap.ts).

You may also find more test-case examples of how the LRU Cache works in [LRUCacheOnMap.test.ts](./__test__/LRUCacheOnMap.test.ts) file.
